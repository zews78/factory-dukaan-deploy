...
exports.getLogin = async (req, res) => {
  ...
  if (page === 1) {
    ...
    const user = await firebase.firestore().collection('users').doc(req.query.uid).get();
...
exports.postLogin = async (req, res) => {
  try {
    ...
    const sessionCookie = await firebase.auth().createSessionCookie(req.body.user.stsTokenManager.accessToken, {
      expiresIn
    });
    const cookieOptions = {
      maxAge: expiresIn,
...
exports.postLogout = async (req, res) => {
  try {
    ...
    await firebase.auth().revokeRefreshTokens(req.uid);
    res.redirect('/');
  } catch (err) {
    res.redirect('/login');
  }
